...
import { ShinyDistribution, ShinyModule, ShinyServer } from '../../common/distribution/DistributionFactory'
import { Type } from 'shiny-types'
...
export class DistributionIndexProcessor extends IndexProcessor {
    ...
    constructor(commonDir: string, protected distribution: ShinyDistribution, protected serverId: string) {
        super(commonDir)
    }
    ...
    public async validate(onStageComplete: () => Promise<void>): Promise<{[category: string]: Asset[]}> {
        ...
        const server: ShinyServer = this.distribution.getServerById(this.serverId)!
        if(server == null) {
    ...
    private async validateModules(modules: ShinyModule[], accumulator: Asset[]): Promise<void> {
    ...
    public async loadVersionManifest<T>(modLoaderModule: ShinyModule): Promise<T> {
