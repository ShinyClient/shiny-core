...
export async function downloadQueue(
    ...
    const receivedTotals: { [id: string]: number } = assets
        .map(({ id }) => id)
    ...
    const wrap = (asset: Asset): Promise<void> =>
        downloadFile(asset.url, asset.path, onEachProgress(asset))

    const q: queueAsPromised<Asset, void> = fastq.promise(wrap, 15)

    const promises: Promise<void>[] = assets
        .map((asset) => q.push(asset))
        .reduce((acc, p) => [...acc, p], [] as Promise<void>[])
...
export async function downloadFile(
    ...
    if (rethrow && error) {
        if (retryCount > MAX_RETRIES) {
            ...
            log.error(
                `Unknown or unretryable exception thrown during request to ${url}. Rethrowing exception.`
            )
        }
