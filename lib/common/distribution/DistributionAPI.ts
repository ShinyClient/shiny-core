...
import { Distribution } from 'shiny-types'
...
import { ShinyDistribution } from './DistributionFactory'
...
export class DistributionAPI {
    ...
    private distribution!: ShinyDistribution
    ...
    public async getDistribution(): Promise<ShinyDistribution> {
        if(this.rawDistribution == null) {
            this.rawDistribution = await this.loadDistribution()
            this.distribution = new ShinyDistribution(this.rawDistribution, this.commonDir, this.instanceDir)
        }
        return this.distribution
    }

    public async getDistributionLocalLoadOnly(): Promise<ShinyDistribution> {
        if(this.rawDistribution == null) {
    ...
    public async refreshDistributionOrFallback(): Promise<ShinyDistribution> {
        ...
        if(distro == null) {
            ...
            this.distribution = new ShinyDistribution(distro, this.commonDir, this.instanceDir)
