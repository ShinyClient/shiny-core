...
export class ShinyServer {
    ...
    private defaultUndefinedJavaOptions(props: JavaVersionProps): Required<JavaVersionProps> {
        return {
            supported:
                props.distribution ?? mcVersionAtLeast('1.17', this.rawServer.minecraftVersion) ? '>=17.x' : '8.x',
            distribution:
                props.distribution ?? process.platform === Platform.DARWIN
...
export class ShinyModule {
    ...
    private resolveRequired(): Required<ShinyRequired> {
