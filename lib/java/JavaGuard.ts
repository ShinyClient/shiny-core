...
export async function getHotSpotSettings(
    execPath: string
): Promise<HotSpotSettings | null> {
    const javaExecutable = execPath.includes('javaw.exe')
        ? execPath.replace('javaw.exe', 'java.exe')
        : execPath
    ...
    try {
        stderr = (
            await execAsync(`"${javaExecutable}" -XshowSettings:properties -version`)
        ).stderr
    } catch (error) {
...
export async function resolveJvmSettings(
    paths: string[]
): Promise<{ [path: string]: HotSpotSettings }> {
...
export function filterApplicableJavaPaths(
    ...
    const jvmDetailsUnfiltered = Object.entries(resolvedSettings)
        ...
        .map(([path, settings]) => {
            ...
            if (parsedVersion == null) {
                log.error(
                    `Failed to parse JDK version at location '${path}' (Vendor: ${settings['java.vendor']})`
                )
                return null!
            }
            return {
    ...
    const jvmDetails = jvmDetailsUnfiltered.filter((details) =>
        semver.satisfies(details.semverStr, semverRange)
    )
...
export async function discoverBestJvmInstallation(
    dataDir: string,
    semverRange: string
): Promise<JvmDetails | null> {
...
export async function validateSelectedJvm(
    path: string,
    semverRange: string
): Promise<JvmDetails | null> {
 ...
 * See: https://github.com/ShinyClient/Shiny/issues/70
...
export async function latestAdoptium(
    ...
    process.platform === Platform.WIN32
        ...
        : process.platform === Platform.DARWIN
            ...
            : process.platform
    const arch: string =
    process.arch === Architecture.ARM64 ? 'aarch64' : Architecture.X64
    ...
    try {
            ...
            if (targetBinary != null) {
                return {
                    ...
                    path: join(
                        ...
                        targetBinary.binary.package.name
                    ),
                }
            } else {
                log.error(
                    `Failed to find a suitable Adoptium binary for JDK ${major} (${sanitizedOS} ${arch}).`
                )
                return null
            }
        } else {
        ...
        log.error(
            `Error while retrieving latest Adoptium JDK ${major} binaries.`,
            err
        )
        return null
    }
...
export async function latestCorretto(
    ...
    try {
        ...
        if (res.statusCode === 200) {
            ...
            log.error(
                `Error while retrieving latest Corretto JDK ${major} (${sanitizedOS} ${arch}): ${
                    res.statusCode
                } ${res.statusMessage ?? ''}`
            )
            return null
...
export async function extractJdk(archivePath: string): Promise<string> {
    ...
    if (archivePath.endsWith('zip')) {
        await extractZip(archivePath, async (zip) => {
            ...
            javaExecPath = javaExecFromRoot(
                join(dirname(archivePath), Object.keys(entries)[0])
            )
        })
    } else {
...
export async function loadMojangLauncherData(): Promise<LauncherJson | null> {
    try {
        const res = await got.get<LauncherJson>(
 ...
 * @returns Object containing the version information.
 */
export function parseJavaRuntimeVersionLegacy(
    verString: string
): JavaVersion | null {
 ...
 * @returns Object containing the version information.
 */
export function parseJavaRuntimeVersionSemver(
    verString: string
): JavaVersion | null {
...
export function javaVersionToString({
    major,
    minor,
    patch,
}: JavaVersion): string {
                            ...
                            if (err) {
                                ...
                                if (javaVers.length === 0) {
                                    ...
                                    for (let j = 0; j < javaVers.length; j++) {
                                        ...
                                        const vKey = javaVer.key
                                            .substring(javaVer.key.lastIndexOf('\\') + 1)
                                            .trim()
...
export async function getValidatableJavaPaths(
    dataDir: string
): Promise<string[]> {
    ...
    switch (process.platform) {
        ...
        default:
            discoverers = []
            log.warn(
                `Unable to discover Java paths on platform: ${process.platform}`
            )
    }
...
export async function getWin32Discoverers(
    dataDir: string
): Promise<JavaDiscoverer[]> {
    return [
...
export async function getDarwinDiscoverers(
    dataDir: string
): Promise<JavaDiscoverer[]> {
    return [
        new EnvironmentBasedJavaDiscoverer(getPossibleJavaEnvs()),
...
export async function getLinuxDiscoverers(
    dataDir: string
): Promise<JavaDiscoverer[]> {
    return [
        new EnvironmentBasedJavaDiscoverer(getPossibleJavaEnvs()),
...
export async function win32DriveMounts(): Promise<string[]> {
    ...
    try {
        stdout = (
            await execAsync(
                'gdr -psp FileSystem | select -eXp root | ConvertTo-Json',
...
export async function getPathsOnAllDrivesWin32(
    paths: string[]
): Promise<string[]> {
